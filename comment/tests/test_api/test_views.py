...
class BaseAPIViewTest(BaseAPITest):
    ...
    def comment_count_test(self):
        self.assertEqual(Comment.objects.filter_parents_by_object(self.post_1).count(),
                         self.parents)
...
class CommentListTest(BaseAPIViewTest):
    ...
    def test_retrieving_with_non_exitsting_model_id(self):
        ...
        self.assertEqual(
            ...
            ContentTypeError.MODEL_ID_INVALID.format(model_id=model_id,
                                                     model_name=url_data["model_name"])
        )
        self.assertTextTranslated(response.data['detail'], url)
...
class CommentCreateTest(BaseAPIViewTest):
    ...
    def test_create_child_comment(self):
        ...
        url_data['parent_id'] = Comment.objects.all_comments_by_object(model_obj).filter(
            parent=None).first().id
...
class CommentDetailForReactionTest(BaseAPIViewTest):
    ...
    def test_cannot_update_comment_content(self):
        ...
        response = self.client.post(self.get_base_url(self.like), data=data,
...
class APIToggleFollowTest(BaseAPITest):
    ...
    def get_base_url(self):
        params = [f'app_name={self.app_name}', f'model_name={self.model_name}',
