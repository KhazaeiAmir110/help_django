...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        self.data = render_to_string(self.get_template_names(), self.get_context_data(),
...
class UpdateComment(CanEditMixin, BaseCommentView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context['comment'] = self.comment
        self.data = render_to_string('comment/comments/update_comment.html', context,
                                     request=self.request)
        return UTF8JsonResponse(self.json())
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            context['comment'] = self.comment
            self.data = render_to_string('comment/comments/comment_content.html', context,
                                         request=self.request)
            return UTF8JsonResponse(self.json())
...
class DeleteComment(CanDeleteMixin, BaseCommentView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.data = render_to_string('comment/comments/comment_modal.html', context,
                                     request=request)
        return UTF8JsonResponse(self.json())
