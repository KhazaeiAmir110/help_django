...
class CommentList(ContentTypeValidator, generics.ListAPIView):
    ...
    def get_queryset(self):
        self.validate(self.request)
        return Comment.objects.filter_parents_by_object(self.model_obj).select_related('user',
                                                                                       'reaction',
                                                                                       'flag')
...
class CommentDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = CommentSerializer
    permission_classes = (
    permissions.IsAuthenticatedOrReadOnly, IsOwnerOrReadOnly, UserPermittedOrReadOnly)
...
class ConfirmComment(APIView, CommentCreateMixin):
    def get(self, request, *args, **kwargs):
        ...
        if temp_comment.why_invalid == CommentFailReason.BAD:
            return Response({'detail': EmailError.BROKEN_VERIFICATION_LINK},
                            status=status.HTTP_400_BAD_REQUEST)

        if temp_comment.why_invalid == CommentFailReason.EXISTS:
            return Response({'detail': EmailError.USED_VERIFICATION_LINK},
