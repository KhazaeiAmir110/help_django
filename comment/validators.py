...
class BaseValidatorMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({'type': ExceptionError.ERROR_TYPE, 'detail': exc.detail},
                                status=400)
        return super().dispatch(request, *args, **kwargs)
...
class ContentTypeValidator(BaseValidatorMixin):
    ...
    def validate_model_object(self, app_name, model_name, model_id):
        ...
        if not model_query.exists() and model_query.count() != 1:
            self.error = ContentTypeError.MODEL_ID_INVALID.format(model_id=model_id,
    ...
    def validate(self, request):
        ...
        self.model_obj = self.validate_model_object(validated_app_name, validated_model_name,
                                                    validated_model_id)
